...
def segment(model, cfg:DictConfig):
    ...
    cfg.mllm.image_path = '/workspace/InpaLLa/results/Tobigs/Tobigs.png' #이걸 이용해서 segment해야 하기 때문에 이미지를 업로드하면 이미지를 path에 저장하도록 해야함
    cfg.mllm.prompt = 'Segment what the character is holding.'
    
    text_output, pred_masks, image_np = seg_image(model, cfg)

    save_path = '/workspace/InpaLLa/results/Tobigs'
